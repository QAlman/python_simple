...
@allure.story('8: 2-927 :   Проверить отчет Пятёрочка - Версия1')
class TestOutsourcing_8(WebBase):

    @allure.title('8: 2-927 :   Проверить отчет Пятёрочка - Версия1')
    @allure.severity(allure.severity_level.CRITICAL)
    @allure.link(name='2-927 :    Проверить отчет Пятёрочка - Версия1',
                 url="https://testlink.verme.ru/linkto.php?tprojectPrefix=2&item=testcase&id=2-927")
    @allure.description('Позитивный тест 2-927 :    Проверить отчет Пятёрочка - Версия1')
    @pytest.mark.CRITICAL
    @pytest.mark.WebTest
    @pytest.mark.test2_927
    #@pytest.mark.skip
